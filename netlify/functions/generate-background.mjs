...
export default async (req) => {
  ...
  // ── Generar con la IA (con reintentos PACIENTES si hay saturación) ──
  ...
  for (let intento = 1; intento <= MAX_INTENTOS; intento++) {
    try {
      ...
      // 429 = "vas muy rápido", 529/500/503 = servidor saturado → ESPERAR y reintentar
      if (response.status === 429 || response.status === 529 || response.status === 500 || response.status === 503) {
        let espera = Math.min(5000 * Math.pow(2, intento - 1), 40000); // 5s, 10s, 20s, 40s
        const ra = response.headers.get('retry-after');
        if (ra && !isNaN(parseInt(ra))) { espera = Math.max(espera, parseInt(ra) * 1000); }
      ...
      if (intento < MAX_INTENTOS) { await new Promise(function(r){ setTimeout(r, 5000 * intento); }); continue; }
    }
  }
